...
const star1 = shader(1.2, 32);
star1.position.set(2, 0, 0);
scene.add(star1);

const star2 = shader(0.5, 32);
star2.position.set(-4, 0, 0);
scene.add(star2);
...
const planet = (size, segments, [name, ext], [x, y, z], isBackSide, isPhong) => {
...
// creating planetes
const galaxy = planet(400, 64, ['galaxy', 'png'], [0, 0, 0], true, false)
scene.add(galaxy)

const rotationAxis = planet(0.001, 16, ['yellow', 'jpg'], [0, 0, 0], false, false)
scene.add(rotationAxis)

const mustafar = planet(0.3, 32, ['mustafar', 'jpg'], [6, 0, 0], false, true)
brightPlanet(mustafar, 4)
rotationAxis.add(mustafar)

const tatooine = planet(0.4, 32, ['tatooine', 'jpg'], [8, 0, 0], false, true)
rotationAxis.add(tatooine)

const alderaan = planet(0.5, 32, ['alderaan', 'jpg'], [10, 0, 0], false, true)
rotationAxis.add(alderaan)

const coruscant = planet(0.4, 32, ['coruscant', 'jpg'], [12, 0, 0], false, true)
brightPlanet(coruscant, 8)
rotationAxis.add(coruscant)

const felucia = planet(0.6, 32, ['felucia', 'jpg'], [14, 0, 0], false, true)
rotationAxis.add(felucia)

const nabooh = planet(0.4, 32, ['nabooh', 'jpeg'], [16, 0, 0], false, true)
rotationAxis.add(nabooh)

const endor = planet(0.15, 32, ['endor', 'jpg'], [18, 0, 0], false, true)
rotationAxis.add(endor)

const deathStar = planet(0.1, 32, ['deathStar', 'jpg'], [20, 0, 0], false, true)
rotationAxis.add(deathStar)

const hoth = planet(0.4, 32, ['hoth', 'png'], [22, 0, 0], false, true)
...
const render = () => {
    ...
    star2.material.uniforms.viewVector.value = new THREE.Vector3().subVectors(camera.position, star2.position)
